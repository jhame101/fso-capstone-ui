...
def convert_serial_to_pd_reading(bytes_in: bytes, previous_time: float) -> tuple[float, float]:
    # return time in ms since board started as an int, and voltage on the pin in volts as a float

    time_since_start = previous_time + (int.from_bytes(bytes_in[0:2], byteorder='little')*1e-4)     # Time is given in intervals of 1e-4 seconds
    voltage_on_pin = int.from_bytes(bytes_in[2:4], byteorder='little')*ARDUINO_ADC_MAX_VOLTAGE/ARDUINO_ADC_MAX_INT

    return time_since_start, voltage_on_pin
...
with serial.Serial(port=ARDUINO_SERIAL_PORT, baudrate=ARDUINO_BAUD_RATE, timeout=TIMEOUT) as arduino:    # implicitly calls arduino.close() afterwards
    ...
    while True:
        ...
        for c in range(NUM_PD_READINGS_PER_CYCLE):
            PD_time, pd_voltages[c] = convert_serial_to_pd_reading(arduino.read(4), PD_time)
            pd_times[c] = PD_time

        sensor_time, humidity, temperature, pressure, light, roll, pitch, yaw = read_values(arduino)
